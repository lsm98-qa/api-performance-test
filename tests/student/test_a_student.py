...
# pytest tests/student/test_student_course.py -v
...
URL = f"https://api-dashboard.elice.io/student/{STUDENT_ID}/course"
...
def get_student_course(headers: dict) -> requests.Response:
...
class TestStudentCoursePositive:
    ...
    def test_get_student_course_with_valid_token(self, valid_headers):
        """
        [요청 조건] 유효한 토큰 포함 GET 요청
        [예상 결과] 200 OK, 학습자 코스 목록 데이터 반환
        """
        # Given: 유효한 토큰이 포함된 헤더가 있을 때

        # When: 학습자 코스 목록 API를 호출한다
        response = get_student_course(valid_headers)

        # Then: 200 OK와 코스 목록 데이터가 반환되어야 한다
...
class TestStudentCourseNegative:
    ...
    def test_get_student_course_without_token(self, no_auth_headers):
        """
        [요청 조건] 토큰 없이 GET 요청
        ...
        # When: 학습자 코스 목록 API를 호출한다
        response = get_student_course(no_auth_headers)
    ...
    def test_get_student_course_with_expired_token(self, expired_headers):
        """
        ...
        # When: 학습자 코스 목록 API를 호출한다
        response = get_student_course(expired_headers)
