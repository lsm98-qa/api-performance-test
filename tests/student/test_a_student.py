...
# ===== 헬퍼 =====
def student_course_endpoint(student_id=STUDENT_ID):
    """학생 과목 목록 조회 API 경로를 만든다."""
...
class TestStudentCoursePositive:
    ...
    def test_get_student_course_with_valid_token(self, dashboard_client):
        # When: 유효한 학습자 토큰으로 학생 과목 목록을 조회한다.
        response = dashboard_client.get(
            student_course_endpoint(),
            params=STUDENT_COURSE_PARAMS,
        )

        # Then: 학생 과목 목록 조회가 정상 처리되어야 한다.
