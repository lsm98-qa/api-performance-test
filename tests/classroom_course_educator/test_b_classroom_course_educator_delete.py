...
class TestClassroomCourseDeleteBoundary:
    ...
    def test_delete_course_with_other_org_header(self, other_org_headers, added_course_id):
        """
        [요청 조건] 다른 기관 헤더(qatrack)로 과목 제거 시도
        [예상 결과] 403 Forbidden, 기관 간 접근 차단
        [실제 결과] 200 OK, JSON 내에서 400 Bad Request를 반환하는 안티 패턴
        """
        # Given: 다른 기관 헤더와 추가된 course_id가 있을 때
