...
# ===== 환경 설정 =====
ORG = os.getenv("ORG_NAME_SHORT_V2")
...
URL = f"https://api-rest.elice.io/org/{ORG}/lecture/edit/"
...
class TestLectureEditBoundary:
    ...
    def test_edit_lecture_with_other_org_token(self, other_org_headers):
        """
        [요청 조건] 다른 기관 토큰으로 강의 수정 시도
        [예상 결과] 403 Forbidden, 기관 간 접근 차단
        [실제 결과] 200 OK, JSON 내에서 400 Bad Request를 반환하는 안티 패턴
        """
        # Given: 다른 기관 토큰과 유효한 강의 수정 데이터가 있을 때

        # When: 다른 기관 토큰으로 강의 수정 API를 호출한다
